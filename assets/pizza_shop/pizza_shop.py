...
def make_brick_mat(name):
    ...
    brick.inputs["Color1"].default_value = (0.55, 0.52, 0.46, 1.0)   # выцветший кирпич
    brick.inputs["Color2"].default_value = (0.44, 0.40, 0.34, 1.0)   # тёмный кирпич
    brick.inputs["Mortar"].default_value = (0.34, 0.33, 0.30, 1.0)   # грязный шов
    ...
    links.new(add.outputs["Value"], mixn.inputs[0])
...
MAT_ROOF      = make_mat("Roof", (0.10, 0.10, 0.10), rough=0.95, grunge=(0.05, 0.05, 0.05))
# Ржавчина: тёмная бурая основа + рыжие пятна коррозии (значения ЛИНЕЙНЫЕ,
# после гаммы выглядят заметно светлее — потому такие тёмные)
MAT_RUST      = make_mat("RustTrim", (0.055, 0.030, 0.018), rough=0.95, grunge=(0.17, 0.085, 0.035), grunge_scale=6)
MAT_SIGNBAND  = make_mat("SignBand", (0.64, 0.62, 0.56), rough=0.85, grunge=(0.34, 0.32, 0.26), grunge_scale=3)
MAT_RED       = make_mat("SignRed", (0.50, 0.04, 0.04), rough=0.55, grunge=(0.30, 0.03, 0.03), grunge_scale=7)
MAT_AWN_RED   = make_mat("AwningRed", (0.40, 0.05, 0.04), rough=0.9, grunge=(0.19, 0.04, 0.03), grunge_scale=5)
MAT_AWN_WHITE = make_mat("AwningWhite", (0.62, 0.58, 0.50), rough=0.9, grunge=(0.36, 0.33, 0.26), grunge_scale=5)
MAT_GLASS     = make_mat("DarkGlass", (0.012, 0.014, 0.016), rough=0.3, metallic=0.1, grunge=(0.05, 0.05, 0.045), grunge_scale=4)
MAT_FRAME     = make_mat("FrameBronze", (0.13, 0.09, 0.06), rough=0.6, metallic=0.5, grunge=(0.07, 0.05, 0.03), grunge_scale=8)
MAT_CONCRETE  = make_mat("Concrete", (0.38, 0.37, 0.35), rough=0.95, grunge=(0.22, 0.21, 0.19), grunge_scale=4)
MAT_ASPHALT   = make_mat("Asphalt", (0.045, 0.045, 0.048), rough=0.95, grunge=(0.02, 0.02, 0.022), grunge_scale=3)
MAT_AC        = make_mat("ACMetal", (0.46, 0.48, 0.48), rough=0.65, metallic=0.4, grunge=(0.24, 0.22, 0.20), grunge_scale=5)
MAT_DARKMETAL = make_mat("DarkMetal", (0.09, 0.09, 0.10), rough=0.5, metallic=0.6)
MAT_GREEN     = make_mat("DumpsterGreen", (0.05, 0.20, 0.07), rough=0.8, grunge=(0.03, 0.10, 0.04), grunge_scale=5)
MAT_YELLOW    = make_mat("BollardYellow", (0.80, 0.60, 0.05), rough=0.7, grunge=(0.42, 0.30, 0.06), grunge_scale=6)
MAT_CHEESE    = make_mat("Cheese", (0.85, 0.64, 0.22), rough=0.65, grunge=(0.62, 0.44, 0.15), grunge_scale=8)
MAT_CRUST     = make_mat("Crust", (0.50, 0.30, 0.11), rough=0.75)
MAT_PEPPERONI = make_mat("Pepperoni", (0.42, 0.07, 0.05), rough=0.6)
MAT_WHITE     = make_mat("SignWhite", (0.78, 0.78, 0.74), rough=0.65, grunge=(0.55, 0.54, 0.50), grunge_scale=6)
...
def cyl(name, r, depth, loc, mat, rot=(0, 0, 0), verts=24, parent=None):
...
for i in range(10):
    ...
    box("Awning_%d" % i, (0.69, 1.2, 0.045), (x, -3.52, 2.36), mat, rot=(radians(20), 0, 0))
...
# ----------------------------------------------------------------------------
# Гранж-детали: мох у основания, мусорные мешки и коробка у баков
# ----------------------------------------------------------------------------
